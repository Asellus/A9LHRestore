...
FLAGS := name=$(name).dat dir_out=$(abspath $(dir_out)) ICON=$(abspath icon.png) APP_DESCRIPTION="ARM9LoaderHax Restor." APP_AUTHOR="Asellus" --no-print-directory
...
.PHONY: a9lh
a9lh: $(dir_out)/$(name).bin

.PHONY: ninjhax
ninjhax: $(dir_out)/3ds/$(name)

.PHONY: release
release: $(dir_out)/$(name)-$(version).zip
...
$(dir_out)/$(name).bin: $(dir_build)/main.bin $(dir_out)
...
$(dir_out)/$(name)-$(version).zip: launcher a9lh ninjhax
	@cd $(dir_out) && zip -9 -r $(name)-$(version).zip *
